    ...
    class Program
    {
        ...
        static void Release(string version, bool overwrite, string comVer)
        {
            try
            {
               ...
               @"C:\GojiLTDTrunk\WCF Streaming\Installer\WCFStreamingServerInstaller\bin\Release\",                           
            };

                string[] installerName = { "WCFStreamingServerInstaller",                                     
                                     };
                for (int i = 0; i < releaseDirectories.Length; i++)
                {
                    using (ZipFile zip = new ZipFile())
                    {
                        ...
                        zip.Comment = "WCF streaming server Version:" + version;
                        zip.Save(installerName[i] + "_" + comVer + "_" + version + ".zip");
                    }
                }


                Console.WriteLine(@"Copy the release to Directory Z:\Rnd\ads\Software\Eli\Febris\Version1.1\Installer\GojiWCFStreamingServer\" + version);
                string finalReleaseDirectory = @"Z:\Rnd\ads\Software\Eli\Febris\Version1.1\Installer\GojiWCFStreamingServer\\" + "Release_" + version;
